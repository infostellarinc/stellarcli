...
var (
	reservePassUse   = util.Normalize("reserve-pass [Reservation Token]")
	reservePassShort = util.Normalize("Reserve a pass for a satellite.")
	reservePassLong  = util.Normalize("Reserve a pass for a satellite. Reservation tokens can be fetched with list-passes command.")
)
...
func NewReservePassCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   reservePassUse,
		Short: reservePassShort,
		Long:  reservePassLong,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			o := &pass.ReservePassOptions{
				Printer:          p,
				ReservationToken: args[0],
			}
